...
public class SwervePIDController extends Command {
  ...
  private PPHolonomicDriveController m_DriveController = new PPHolonomicDriveController(kTranslationPID, kRotationPID);
  ...
  private final Debouncer endDebouncer = new Debouncer(kEndDebounce.in(Seconds), Debouncer.DebounceType.kBoth);
  private final BooleanPublisher endConditionLogger = ntInst.getTable("swerve").getBooleanTopic("PIDEndCondition")
      .publish();
  ...
  private static final StructSubscriber<Pose2d> driveStatePose = driveStateTable.getStructTopic("Pose", Pose2d.struct)
      .subscribe(new Pose2d());
  private final StructSubscriber<ChassisSpeeds> driveSpeeds = driveStateTable
      .getStructTopic("Speeds", ChassisSpeeds.struct).subscribe(new ChassisSpeeds());

  private DoublePublisher vxPub = ntInst.getTable("swerve/PID").getDoubleTopic("vxMps").publish();
  private DoublePublisher vyPub = ntInst.getTable("swerve/PID").getDoubleTopic("vyMps").publish();
  private DoublePublisher omegaPub = ntInst.getTable("swerve/PID").getDoubleTopic("omegaRps").publish();
  private DoublePublisher errorPub = ntInst.getTable("swerve/PID").getDoubleTopic("error").publish();
  ...
  public void execute() {
    ...
    // Calculate theoretical PID chassis speeds to get to our goal (assumes no max
    ...
    // Constrain the acceleration to what the robot can actually do (with no/little
    // wheel slip)
    ...
    // m_swerve.driveRobotRelative(speeds); // TODO: swerve setpoint generator
    // testing
  }
  ...
  public boolean isFinished() {
    ...
    // DataLogManager.log(String.format("exp: %s act: %s tol: %s",
    // m_goalPose.getRotation( ).getRotations( ),
    // driveStatePose.get( ).getRotation( ).getRotations( ),
